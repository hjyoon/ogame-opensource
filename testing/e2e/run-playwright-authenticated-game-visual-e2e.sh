...
OUTPUT_ROOT="${OGAME_GAME_VISUAL_OUTPUT_ROOT:-$ROOT_DIR/.tmp/playwright-authenticated-game-visual}"
...
prepare_fixture() {
  ...
  docker compose exec -T \
    ...
    -e OGAME_GAME_VISUAL_PREMIUM_FIXTURE="${OGAME_GAME_VISUAL_PREMIUM_FIXTURE:-0}" \
...
for browser in $BROWSERS; do
  ...
  (
    ...
    OGAME_GAME_VISUAL_OUTPUT_DIR="$OUTPUT_ROOT/$browser" \
