...
db_query "DROP TABLE IF EXISTS $queue_backup,$user_backup,$ally_backup,$uni_backup,$planet_backup,$botvar_backup,$debug_backup;
CREATE TABLE $queue_backup AS SELECT * FROM uni1_queue WHERE type IN ($queue_types);
CREATE TABLE $user_backup AS SELECT player_id,session,private_session,lastlogin,lastclick,ip_addr,score1,score2,score3,place1,place2,place3,oldscore1,oldscore2,oldscore3,oldplace1,oldplace2,oldplace3,scoredate,admin FROM uni1_users;
...
CREATE TABLE $planet_backup AS SELECT * FROM uni1_planets WHERE type IN (10000,20000);
CREATE TABLE $botvar_backup AS SELECT * FROM uni1_botvars WHERE owner_id=$bot_id;
CREATE TABLE $debug_backup AS SELECT * FROM uni1_debug" >/dev/null

restore_case() {
  db_query "DELETE FROM uni1_queue WHERE type IN ($queue_types); INSERT INTO uni1_queue SELECT * FROM $queue_backup;
...
DELETE FROM uni1_planets WHERE type IN (10000,20000); INSERT INTO uni1_planets SELECT * FROM $planet_backup;
DELETE FROM uni1_botvars WHERE owner_id=$bot_id; INSERT INTO uni1_botvars SELECT * FROM $botvar_backup;
DELETE FROM uni1_debug; INSERT INTO uni1_debug SELECT * FROM $debug_backup" >/dev/null
}

cleanup() {
  restore_case >/dev/null 2>&1 || true
  db_query "DROP TABLE IF EXISTS $queue_backup,$user_backup,$ally_backup,$uni_backup,$planet_backup,$botvar_backup,$debug_backup" >/dev/null 2>&1 || true
  rm -rf "$TMP_DIR"
}
...
run_side() {
  ...
  if [ "$side" = legacy ]; then
    http="$(curl --silent --show-error --max-time 90 --output "$TMP_DIR/$side-$case_name.body" --cookie "$TMP_DIR/legacy.cookies" \
    ...
    http="$(curl --silent --show-error --max-time 90 --output "$TMP_DIR/$side-$case_name.body" --header 'Content-Type: application/json' \
...
cases='unload-all clean-debris update-stats recalc-points recalc-alliance ai'
