...
ELAPSED_SECONDS="${OGAME_FLEET_RECALL_ELAPSED_SECONDS:-600}"
...
prepare_fixture() {
  ...
  docker compose exec -T \
    -e OGAME_OVERVIEW_FLEET_OWN_ELAPSED_SECONDS="$ELAPSED_SECONDS" \
...
for browser in ${OGAME_FLEET_RECALL_TIMING_BROWSERS:-chromium firefox}; do
  prepare_fixture
  printf 'Fleet recall timing E2E (%s, elapsed=%ss)\n' "$browser" "$ELAPSED_SECONDS"
  OGAME_PLAYWRIGHT_BROWSER="$browser" \
  OGAME_GO_BASE_URL="$GO_BASE_URL" \
  OGAME_OVERVIEW_FLEET_FIXTURE_FILE="$FIXTURE_FILE" \
  bun run e2e:fleet-recall-timing
done
