...
DYNAMIC_FIXTURE_COMMANDER="${OGAME_GAME_VISUAL_COMMANDER_FIXTURE:-1}"
DYNAMIC_FIXTURE_ALLIANCE="${OGAME_GAME_VISUAL_ALLIANCE_FIXTURE:-1}"
...
for browser in $BROWSERS; do
  if [ "${OGAME_GAME_DYNAMIC_PREPARE_FIXTURE:-1}" = "1" ]; then
    ...
    docker compose exec -T \
      ...
      -e OGAME_GAME_VISUAL_ALLIANCE_FIXTURE="$DYNAMIC_FIXTURE_ALLIANCE" \
    ...
    docker compose -f "$ROOT_DIR/docker-compose.yml" exec -T goapp sh -c \
  ...
  (
    ...
    OGAME_GAME_VISUAL_COMMANDER_FIXTURE="$DYNAMIC_FIXTURE_COMMANDER" \
    OGAME_GAME_VISUAL_ALLIANCE_FIXTURE="$DYNAMIC_FIXTURE_ALLIANCE" \
