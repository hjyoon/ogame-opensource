...
| Galaxy hover/actions | `galaxy.php`, `galaxy_js.php` overLib menus, `doit`, cursor keys | Partially migrated | hover visual, action navigation, instant spy/recycle dispatch, galaxy HTTP tests | keyboard navigation and failure dispatch variants need behavior assertions |
...
`run-playwright-authenticated-game-dynamic-e2e.sh` runs 17 shared-fixture cases:
message/notes/buddy/alliance/application counters, galaxy tooltip/action
navigation/instant dispatch, fleet all-ships, `shortInfo`, residue/overcapacity,
merchant max/negative clamps, and BattleSim slot sync. Merchant text allows
