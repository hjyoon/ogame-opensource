...
| Galaxy hover/actions | `galaxy.php`, `galaxy_js.php` overLib menus, `doit`, cursor keys | Partially migrated | hover, action nav, keyboard, instant dispatch, galaxy HTTP tests | failure dispatch variants need behavior assertions |
...
`run-playwright-authenticated-game-dynamic-e2e.sh` runs 21 shared-fixture cases:
