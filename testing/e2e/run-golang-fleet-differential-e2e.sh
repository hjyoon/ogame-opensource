...
capture_state() {
  ...
  logs="$(db_query "SELECT IF(COUNT(*)=0,'[]',JSON_ARRAYAGG(JSON_OBJECT('ownerId',owner_id,'targetId',target_id,'unionId',union_id,'mission',mission,'flightTime',flight_time,'deployTime',deploy_time,'duration',end-start,'fuel',fuel,'origin',CONCAT(origin_g,':',origin_s,':',origin_p,':',origin_type),'target',CONCAT(target_g,':',target_s,':',target_p,':',target_type),'metal',ROUND(\`700\`),'crystal',ROUND(\`701\`),'deuterium',ROUND(\`702\`),'smallCargo',\`202\`))) FROM (SELECT * FROM uni1_fleetlogs WHERE owner_id IN ($players) OR target_id IN ($players) ORDER BY log_id) rows_ordered")"
...
legacy_launch() {
  ...
  legacy_launch_status="$(curl --silent --show-error --max-time 15 --output "$TMP_DIR/$prefix-launch.body" --cookie "$TMP_DIR/$prefix.cookies" --write-out '%{http_code}' \
    ...
    --data-urlencode "galaxy=$tg" --data-urlencode "system=$ts" --data-urlencode "planet=$tp" --data-urlencode 'planettype=1' --data-urlencode 'speed=10' --data-urlencode "order=$current_mission" \
    --data-urlencode "ship202=$current_ships" --data-urlencode 'ship203=0' --data-urlencode 'ship204=0' --data-urlencode 'ship205=0' --data-urlencode 'ship206=0' --data-urlencode 'ship207=0' --data-urlencode 'ship208=0' --data-urlencode 'ship209=0' --data-urlencode 'ship210=0' --data-urlencode 'ship211=0' --data-urlencode 'ship212=0' --data-urlencode 'ship213=0' --data-urlencode 'ship214=0' --data-urlencode 'ship215=0' \
...
go_launch() {
  ...
  payload="$(jq -nc --arg g "$1" --arg s "$2" --arg p "$3" --arg mission "$current_mission" --arg ships "$current_ships" --arg m "$current_metal" --arg c "$current_crystal" --arg d "$current_deuterium" '{action:"launch-dispatch",ships:{"202":($ships|tonumber)},resources:{"700":($m|tonumber),"701":($c|tonumber),"702":($d|tonumber)},target:{galaxy:($g|tonumber),system:($s|tonumber),position:($p|tonumber)},targetType:1,mission:($mission|tonumber),speed:10}')"
...
run_side() {
  ...
  if [ "$mode" = recall ]; then
    ...
    transitioned="$(capture_state)"
    [ "$mode" = transport ] && force_latest_due
  fi
  if [ "$side" = legacy ]; then
...
run_case() {
  mode="$1"
  case "$mode" in
    transport) current_target_id="$transport_target_id"; current_mission=3; current_ships=1; current_metal=123; current_crystal=45; current_deuterium=0 ;;
    recall) current_target_id="$transport_target_id"; current_mission=3; current_ships=1; current_metal=88; current_crystal=33; current_deuterium=0 ;;
    deploy) current_target_id="$deploy_target_id"; current_mission=4; current_ships=2; current_metal=77; current_crystal=22; current_deuterium=11 ;;
  esac
  ...
  jq -e '.fleet != null and .queue != null and .counts.fleets == 1 and .counts.tasks == 1 and .origin.smallCargo < $before.origin.smallCargo and .origin.metal < $before.origin.metal' --argjson before "$legacy_before" >/dev/null <<EOF || pass=false
$legacy_launched
EOF
...
EOF
  if [ "$mode" = deploy ]; then
    jq -e '.target.smallCargo == 2 and .target.metal >= 1000077 and .target.crystal >= 1000022 and .target.deuterium >= 1000011' >/dev/null <<EOF || pass=false
$legacy_final
EOF
  else
    jq -e '.origin.smallCargo == 10' >/dev/null <<EOF || pass=false
$legacy_final
EOF
  fi
...
[ "$all_pass" = true ]
printf 'Go/PHP fleet differential E2E: PASS (3 cases)\n'
