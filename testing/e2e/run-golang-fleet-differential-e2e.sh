...
FLEET_CASES="${OGAME_FLEET_DIFFERENTIAL_CASES:-transport recall deploy recycle spy attack attack-defense attack-guarded-win attack-defense-repair acs-hold}"
...
for fleet_case in $FLEET_CASES; do
  case "$fleet_case" in
    transport|recall|deploy|recycle|spy|attack|attack-defense|attack-guarded-win|attack-defense-repair|acs-hold) ;;
