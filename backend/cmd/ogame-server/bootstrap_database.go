...
func openSQLiteDatabasePools(cfg config.Config, logger *slog.Logger, pools databasePools) databasePools {
	options := sqlitedb.BootstrapOptions{
		...
		Settings: &sqlitedb.UniverseSettings{
			Language:        cfg.UniLanguage,
			Speed:           cfg.UniSpeed,
			FleetSpeed:      cfg.UniFleetSpeed,
			Galaxies:        cfg.UniGalaxies,
			Systems:         cfg.UniSystems,
			MaxUsers:        cfg.UniMaxUsers,
			StartDarkMatter: cfg.UniStartDarkMatter,
			ACS:             cfg.UniACS,
			FID:             cfg.UniFID,
			DID:             cfg.UniDID,
			RapidFire:       cfg.UniRapidFire,
			Moons:           cfg.UniMoons,
			BattleEngine:    cfg.UniBattleEngine,
			PHPBattle:       cfg.UniPHPBattle,
			BattleMax:       cfg.UniBattleMax,
			ForceLanguage:   cfg.UniForceLanguage,
			MaxShipyard:     cfg.UniMaxShipyard,
			FeedAge:         cfg.UniFeedAge,
			ExtBoard:        cfg.ExtBoard,
			ExtDiscord:      cfg.ExtDiscord,
			ExtTutorial:     cfg.ExtTutorial,
			ExtRules:        cfg.ExtRules,
			ExtImpressum:    cfg.ExtImpressum,
		},
	}
