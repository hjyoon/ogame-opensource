...
func validateFleetDispatchTool() domainmcp.Tool {
	return domainmcp.Tool{
		...
		Description: "Dry-run a fleet dispatch against the authenticated player's current fleet screen and return readiness, issue, fuel, cargo, timing, and confirmation data. Expedition holdSeconds includes the universe fleet-speed factor.",
		InputSchema: map[string]any{
			"type": "object",
			"properties": map[string]any{
				...
				"holdHours": map[string]any{
					"type":        "integer",
					"minimum":     0,
					"description": "ACS hold duration in logical hours. Ignored for other missions.",
				},
				"expeditionHours": map[string]any{
					"type":        "integer",
					"minimum":     0,
					"description": "Expedition stay in logical hours, clamped to Expedition Technology. Actual holdSeconds is divided by the universe fleet-speed factor.",
				},
			...
			"properties": map[string]any{
				"fleetDispatchValidation": map[string]any{
					...
					"properties": map[string]any{
						...
						"speedFactor":          map[string]any{"type": "integer"},
						"holdHours":            map[string]any{"type": "integer"},
						"holdSeconds":          map[string]any{"type": "integer"},
					...
					"required": []string{"playerId", "planetId", "ready", "dryRun", "requiresConfirmation", "totalShips", "mission", "target", "targetType", "speedFactor", "holdHours", "holdSeconds"},
				},
			},
...
func dispatchFleetTool() domainmcp.Tool {
	return domainmcp.Tool{
		...
		Description: "Execute a fleet dispatch only with the exact confirmation returned by validate_fleet_dispatch for the same arguments. Expedition holdSeconds includes the universe fleet-speed factor.",
		InputSchema: map[string]any{
			"type": "object",
			"properties": map[string]any{
				"planetId":       map[string]any{"type": "integer", "minimum": 1},
				...
				"expeditionHours": map[string]any{
					"type":        "integer",
					"minimum":     0,
					"description": "Expedition stay in logical hours. Must match the value used for validation.",
				},
				"unionId": map[string]any{"type": "integer", "minimum": 0},
		...
		OutputSchema: map[string]any{
			...
			"properties": map[string]any{
				"fleetDispatch": map[string]any{
					...
					"required": []string{"playerId", "planetId", "ready", "dryRun", "executed", "totalShips", "mission", "target", "targetType", "speedFactor", "holdHours", "holdSeconds"},
				},
			},
