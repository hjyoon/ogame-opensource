...
func TestServiceListsStatisticsToolForReadScope(t *testing.T) {
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
		},
	}).WithStatisticsReadRepository(&fakeStatisticsReadRepository{})
...
func TestServiceListsBuddyMutationToolForBuddyWriteScope(t *testing.T) {
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"buddy-write": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeBuddyWrite}},
		},
	}).WithBuddyWriteRepository(&fakeBuddyWriteRepository{})

	tools, err := service.ListTools(context.Background(), domainmcp.ListToolsCommand{AccessToken: "buddy-write"})
	if err != nil {
		t.Fatalf("ListTools returned error: %v", err)
	}
	names := make([]string, 0, len(tools.Tools))
	...
	if strings.Join(names, ",") != "get_server_health,mutate_buddy" {
		t.Fatalf("unexpected buddy-write tools: %v", names)
	}
}
...
func TestServiceListsMerchantStatusToolForReadScope(t *testing.T) {
	...
	}).WithMerchantReadRepository(&fakeMerchantReadRepository{})
...
func TestServiceListsShipyardOptionsToolForReadScope(t *testing.T) {
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
		},
	}).WithShipyardOptionsReadRepository(&fakeShipyardOptionsReadRepository{})
...
func TestServiceCallsStatisticsTool(t *testing.T) {
	statistics := domainmcp.Statistics{
		PlayerID:         42,
		PlanetID:         99,
		ViewerAllianceID: 7,
		Who:              "player",
		...
		Total:            250,
		GeneratedAt:      123456,
		Rows: []domainmcp.StatisticsRow{{
			Place:        101,
			DisplayScore: 950000,
			Player:       &domainmcp.StatisticsPlayerRef{ID: 42, Name: "legor"},
			Alliance:     &domainmcp.StatisticsAllianceRef{ID: 7, Tag: "TAG"},
			Own:          true,
		}},
	}
	repository := &fakeStatisticsReadRepository{result: statistics}
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
		},
	}).WithStatisticsReadRepository(repository)

	result, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{
		Name:        "get_statistics",
		AccessToken: "read",
		Arguments:   map[string]any{"planetId": "99", "who": "player", "type": "resources", "start": 101},
	})
	if err != nil {
		t.Fatalf("CallTool returned error: %v", err)
	}
	got := result.StructuredContent.(map[string]any)["statistics"].(domainmcp.Statistics)
	if result.IsError || got.PlayerID != 42 || got.Type != "ressources" || len(got.Rows) != 1 || got.Rows[0].Player.Name != "legor" {
		t.Fatalf("unexpected statistics result: %+v", result)
	}
	if repository.playerID != 42 || repository.command.PlanetID != 99 || repository.command.Who != "player" || repository.command.Type != "ressources" || repository.command.Start != 101 {
		t.Fatalf("unexpected statistics command: player=%d command=%+v", repository.playerID, repository.command)
	}
}

func TestServiceStatisticsToolRequiresRepositoryReadScopeAndValidArguments(t *testing.T) {
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read":  {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
			"write": {Authenticated: true, PlayerID: 43, Scopes: []string{domainmcp.ScopeWrite}},
	...
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_statistics", AccessToken: "read"}); err == nil {
		t.Fatalf("expected missing statistics read repository error")
	}
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_statistics", AccessToken: "write"}); !errors.Is(err, domainmcp.ErrForbidden) {
		t.Fatalf("expected forbidden without read scope, got %v", err)
	}

	service = service.WithStatisticsReadRepository(&fakeStatisticsReadRepository{})
	for _, arguments := range []map[string]any{
		{"planetId": true},
		{"who": true},
		{"who": "bad"},
		...
		if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_statistics", AccessToken: "read", Arguments: arguments}); !errors.Is(err, domainmcp.ErrInvalidParams) {
			t.Fatalf("expected invalid params for %+v, got %v", arguments, err)
		}
	}

	service = service.WithStatisticsReadRepository(&fakeStatisticsReadRepository{err: errors.New("statistics down")})
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_statistics", AccessToken: "read"}); err == nil || !strings.Contains(err.Error(), "statistics down") {
		t.Fatalf("expected statistics repository error, got %v", err)
	}
...
func TestServiceCallsBuddyMutationWithDryRunAndConfirmation(t *testing.T) {
	status := domainmcp.BuddyStatus{PlayerID: 42, Planet: domainmcp.Planet{ID: 99, Name: "Arakis"}, Rows: []domainmcp.BuddyRow{}}
	repository := &fakeBuddyWriteRepository{
		preview: domainmcp.BuddyMutationResult{PlayerID: 42, PlanetID: 99, Action: "add", LegacyAction: 1, BuddyID: 77, TextChars: 5, Status: status},
		mutated: domainmcp.BuddyMutationResult{PlayerID: 42, PlanetID: 99, Action: "add", LegacyAction: 1, BuddyID: 77, TextChars: 5, Status: status},
	}
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"buddy-write": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeBuddyWrite}},
		},
	}).WithBuddyWriteRepository(repository)

	result, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{
		Name:        "mutate_buddy",
		AccessToken: "buddy-write",
		Arguments:   map[string]any{"planetId": 99, "action": "add", "buddyId": 77, "text": "hello"},
	})
	if err != nil {
		t.Fatalf("mutate_buddy dry-run returned error: %v", err)
	}
	preview := result.StructuredContent.(map[string]any)["buddyMutation"].(domainmcp.BuddyMutationResult)
	if !preview.DryRun || preview.Executed || !preview.RequiresConfirmation || !strings.HasPrefix(preview.Confirmation, "mutate_buddy:99:add:77:") {
		t.Fatalf("unexpected buddy preview: %+v", preview)
	}
	if repository.previewPlayerID != 42 || repository.previewCommand.Action != "add" || repository.previewCommand.BuddyID != 77 || !repository.previewCommand.DryRun {
		t.Fatalf("unexpected buddy preview command: player=%d command=%+v", repository.previewPlayerID, repository.previewCommand)
	}

	result, err = service.CallTool(context.Background(), domainmcp.CallToolCommand{
		Name:        "mutate_buddy",
		AccessToken: "buddy-write",
		Arguments:   map[string]any{"planetId": 99, "action": "add", "buddyId": 77, "text": "hello", "dryRun": false, "confirm": preview.Confirmation},
	})
	if err != nil {
		t.Fatalf("mutate_buddy execute returned error: %v", err)
	}
	mutated := result.StructuredContent.(map[string]any)["buddyMutation"].(domainmcp.BuddyMutationResult)
	if mutated.DryRun || !mutated.Executed || mutated.RequiresConfirmation || mutated.Confirmation != preview.Confirmation {
		t.Fatalf("unexpected buddy execute: %+v", mutated)
	}
	if repository.mutatePlayerID != 42 || repository.mutateCommand.Confirm != preview.Confirmation || repository.mutateCommand.DryRun {
		t.Fatalf("unexpected buddy mutate command: player=%d command=%+v", repository.mutatePlayerID, repository.mutateCommand)
	}
}

func TestServiceBuddyMutationIssueDoesNotRequireConfirmation(t *testing.T) {
	repository := &fakeBuddyWriteRepository{
		preview: domainmcp.BuddyMutationResult{
			PlayerID: 42,
			Action:   "add",
			BuddyID:  77,
			Issue:    &domainmcp.ActionIssue{Code: "already_sent", Message: "duplicate"},
		},
	}
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
	...
	result, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{
		Name:        "mutate_buddy",
		AccessToken: "buddy-write",
		Arguments:   map[string]any{"action": "add", "buddyId": 77},
	})
	if err != nil {
		t.Fatalf("mutate_buddy issue dry-run returned error: %v", err)
	}
	preview := result.StructuredContent.(map[string]any)["buddyMutation"].(domainmcp.BuddyMutationResult)
	if !preview.DryRun || preview.RequiresConfirmation || preview.Confirmation != "" || preview.Issue == nil {
		t.Fatalf("expected issue without confirmation, got %+v", preview)
	}
}

func TestServiceBuddyMutationRequiresScopeRepositoryAndValidConfirmation(t *testing.T) {
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read":        {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
			"buddy-write": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeBuddyWrite}},
	...
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "mutate_buddy", AccessToken: "buddy-write", Arguments: map[string]any{"action": "add", "buddyId": 77}}); err == nil {
		t.Fatalf("expected missing buddy write repository error")
	}

	service = service.WithBuddyWriteRepository(&fakeBuddyWriteRepository{})
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "mutate_buddy", AccessToken: "read", Arguments: map[string]any{"action": "add", "buddyId": 77}}); !errors.Is(err, domainmcp.ErrForbidden) {
		t.Fatalf("expected forbidden without buddy write scope, got %v", err)
	}
	for _, arguments := range []map[string]any{
		nil,
		{"action": "unknown", "buddyId": 77},
		{"action": true, "buddyId": 77},
		{"action": "add", "buddyId": 0},
		{"action": "add", "buddyId": true},
		{"action": "add", "buddyId": 77, "text": true},
		{"action": "add", "buddyId": 77, "dryRun": "no"},
		{"action": "add", "buddyId": 77, "confirm": true},
		{"action": "add", "buddyId": 77, "dryRun": false},
		{"action": "add", "buddyId": 77, "dryRun": false, "confirm": "wrong"},
	} {
		if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "mutate_buddy", AccessToken: "buddy-write", Arguments: arguments}); !errors.Is(err, domainmcp.ErrInvalidParams) {
			t.Fatalf("expected invalid params for %+v, got %v", arguments, err)
		}
	}

	service = service.WithBuddyWriteRepository(&fakeBuddyWriteRepository{err: errors.New("buddy write down")})
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "mutate_buddy", AccessToken: "buddy-write", Arguments: map[string]any{"action": "add", "buddyId": 77}}); err == nil || !strings.Contains(err.Error(), "buddy write down") {
		t.Fatalf("expected buddy write repository error, got %v", err)
	}
...
func TestServiceCallsMerchantStatusTool(t *testing.T) {
	status := domainmcp.MerchantStatus{
		PlayerID:      42,
		Planet:        domainmcp.Planet{ID: 99, Name: "Arakis"},
		Commander:     "legor",
		User:          domainmcp.MerchantUser{PaidDarkMatter: 4000, FreeDarkMatter: 7000},
		ActiveOfferID: 1,
		Rows:          []domainmcp.MerchantResourceRow{{ID: 1, Name: "Metal", Offered: true}},
	}
	repository := &fakeMerchantReadRepository{result: status}
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
		},
	}).WithMerchantReadRepository(repository)
	...
	got := result.StructuredContent.(map[string]any)["merchantStatus"].(domainmcp.MerchantStatus)
	if result.IsError || got.PlayerID != 42 || got.ActiveOfferID != 1 || len(got.Rows) != 1 {
		t.Fatalf("unexpected merchant status result: %+v", result)
	}
	if repository.playerID != 42 || repository.command.PlanetID != 99 {
		t.Fatalf("unexpected merchant status command: player=%d command=%+v", repository.playerID, repository.command)
	}
}

func TestServiceMerchantStatusToolRequiresRepositoryReadScopeAndValidArguments(t *testing.T) {
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read":  {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
			"write": {Authenticated: true, PlayerID: 43, Scopes: []string{domainmcp.ScopeWrite}},
	...
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_merchant_status", AccessToken: "read"}); err == nil {
		t.Fatalf("expected missing merchant read repository error")
	}
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_merchant_status", AccessToken: "write"}); !errors.Is(err, domainmcp.ErrForbidden) {
		t.Fatalf("expected forbidden without read scope, got %v", err)
	}

	service = service.WithMerchantReadRepository(&fakeMerchantReadRepository{})
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_merchant_status", AccessToken: "read", Arguments: map[string]any{"planetId": true}}); !errors.Is(err, domainmcp.ErrInvalidParams) {
		t.Fatalf("expected invalid params, got %v", err)
	}
...
func TestServiceCallsShipyardOptionsTool(t *testing.T) {
	options := domainmcp.ShipyardOptions{
		PlayerID:    42,
		Planet:      domainmcp.Planet{ID: 99, Name: "Arakis"},
		HasShipyard: true,
	...
	repository := &fakeShipyardOptionsReadRepository{result: options}
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read": {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
		},
	}).WithShipyardOptionsReadRepository(repository)
	...
	got := result.StructuredContent.(map[string]any)["shipyardOptions"].(domainmcp.ShipyardOptions)
	if result.IsError || got.PlayerID != 42 || got.Planet.ID != 99 || len(got.Items) != 1 || got.Items[0].Name != "Small Cargo" {
		t.Fatalf("unexpected shipyard options result: %+v", result)
	}
	if repository.playerID != 42 || repository.planetID != 99 {
		t.Fatalf("unexpected shipyard options command: player=%d planet=%d", repository.playerID, repository.planetID)
	}
}

func TestServiceShipyardOptionsToolRequiresRepositoryReadScopeAndValidArguments(t *testing.T) {
	service := NewServiceWithTokenVerifier(fakeHealthProvider{}, fakeTokenVerifier{
		access: map[string]domainmcp.Access{
			"read":  {Authenticated: true, PlayerID: 42, Scopes: []string{domainmcp.ScopeRead}},
			"write": {Authenticated: true, PlayerID: 43, Scopes: []string{domainmcp.ScopeWrite}},
	...
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_shipyard_options", AccessToken: "read"}); err == nil {
		t.Fatalf("expected missing shipyard read repository error")
	}
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "get_shipyard_options", AccessToken: "write"}); !errors.Is(err, domainmcp.ErrForbidden) {
		t.Fatalf("expected forbidden without read scope, got %v", err)
	}

	service = service.WithShipyardOptionsReadRepository(&fakeShipyardOptionsReadRepository{})
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{
		Name:        "get_shipyard_options",
		AccessToken: "read",
		Arguments:   map[string]any{"planetId": true},
...
func TestServiceMessageToolsRequireMessageScopeRepositoryAndValidArguments(t *testing.T) {
	...
	if _, err := service.CallTool(context.Background(), domainmcp.CallToolCommand{Name: "list_messages", AccessToken: "messages", Arguments: map[string]any{"cursor": "invalid"}}); !errors.Is(err, domainmcp.ErrInvalidParams) {
...
func TestMCPMessageQueryDefaultsCapsAndValidation(t *testing.T) {
	...
	for _, cursor := range []any{"", "not-base64", base64.RawURLEncoding.EncodeToString([]byte("v2:1700:77")), base64.RawURLEncoding.EncodeToString([]byte("v1:-1:77")), base64.RawURLEncoding.EncodeToString([]byte("v1:1700:0")), true} {
		if _, err := mcpMessageQuery(map[string]any{"cursor": cursor}); !errors.Is(err, domainmcp.ErrInvalidParams) {
			t.Fatalf("expected invalid message cursor for %v, got %v", cursor, err)
		}
	}
}
...
type fakeStatisticsReadRepository struct {
	result   domainmcp.Statistics
	playerID int
	command  domainmcp.StatisticsCommand
	err      error
}

func (f *fakeStatisticsReadRepository) GetMCPStatistics(_ context.Context, playerID int, command domainmcp.StatisticsCommand) (domainmcp.Statistics, error) {
	f.playerID = playerID
	f.command = command
	if f.err != nil {
...
type fakeBuddyWriteRepository struct {
	preview         domainmcp.BuddyMutationResult
	mutated         domainmcp.BuddyMutationResult
	previewPlayerID int
	mutatePlayerID  int
	previewCommand  domainmcp.BuddyMutationCommand
	mutateCommand   domainmcp.BuddyMutationCommand
	err             error
}

func (f *fakeBuddyWriteRepository) PreviewMCPBuddyMutation(_ context.Context, playerID int, command domainmcp.BuddyMutationCommand) (domainmcp.BuddyMutationResult, error) {
	f.previewPlayerID = playerID
	f.previewCommand = command
	if f.err != nil {
...
func (f *fakeBuddyWriteRepository) MutateMCPBuddy(_ context.Context, playerID int, command domainmcp.BuddyMutationCommand) (domainmcp.BuddyMutationResult, error) {
...
type fakeMerchantReadRepository struct {
	result   domainmcp.MerchantStatus
	playerID int
	command  domainmcp.MerchantStatusCommand
	err      error
}

func (f *fakeMerchantReadRepository) GetMCPMerchantStatus(_ context.Context, playerID int, command domainmcp.MerchantStatusCommand) (domainmcp.MerchantStatus, error) {
	f.playerID = playerID
	f.command = command
	if f.err != nil {
...
type fakeShipyardOptionsReadRepository struct {
	result   domainmcp.ShipyardOptions
	playerID int
	planetID int
	err      error
}

func (f *fakeShipyardOptionsReadRepository) GetMCPShipyardOptions(_ context.Context, playerID int, planetID int) (domainmcp.ShipyardOptions, error) {
	f.playerID = playerID
	f.planetID = planetID
	if f.err != nil {
