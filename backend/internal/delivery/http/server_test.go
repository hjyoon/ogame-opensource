...
func TestFrontendServesIndexAndSpaFallback(t *testing.T) {
	...
	for _, target := range []string{"/", "/game/overview", "/game/ainfo.php?allyid=7"} {
...
func TestLegacyAssetAliasesServeStaticFiles(t *testing.T) {
	...
	for _, target := range []string{"/evolution/missing.css", "/game/js/app.js"} {
		req := httptest.NewRequest(http.MethodGet, target, nil)
		rec := httptest.NewRecorder()
		server.ServeHTTP(rec, req)
		if rec.Code != http.StatusNotFound {
...
func TestGameOverviewEndpointReturnsOverview(t *testing.T) {
		...
		Overview: domaingame.Overview{
			...
			CurrentPlanet: domaingame.PlanetOverview{
				...
				Resources: domaingame.Resources{
					...
					Crystal:           234.5,
					Deuterium:         12,
					ProductionPerHour: domaingame.ResourceProductionValues{Metal: 321, Crystal: 123, Deuterium: 45},
					DarkMatter:        37,
					Energy:            140,
					EnergyCapacity:    162,
	...
	if response.Overview.Score.Points != 123 ||
		...
		response.Overview.CurrentPlanet.Resources.ProductionPerHour.Metal != 321 ||
		response.Overview.CurrentPlanet.Resources.ProductionPerHour.Crystal != 123 ||
		response.Overview.CurrentPlanet.Resources.ProductionPerHour.Deuterium != 45 ||
...
func TestGameGalaxyEndpointReturnsGalaxy(t *testing.T) {
		...
		Galaxy: domaingame.Galaxy{
			...
			Rows: []domaingame.GalaxyRow{{
				...
				Planet: &domaingame.GalaxyPlanet{
					...
					ReportID: 701,
					Actions:  domaingame.GalaxyActions{Spy: true, Message: true, Buddy: true, ViewReport: true, Phalanx: true},
				},
	...
	if response.Galaxy.Rows[0].Planet == nil || response.Galaxy.Rows[0].Planet.Player == nil ||
		...
		!response.Galaxy.Rows[0].Planet.Actions.ViewReport || !response.Galaxy.Rows[0].Planet.Actions.Phalanx ||
		response.Galaxy.Rows[0].Debris.Harvesters != 1 {
