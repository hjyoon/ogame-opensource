...
func TestStatisticsRepositoryKeepsAllianceModePendingWithoutPlayerRows(t *testing.T) {
	queryer := &fakeQueryer{results: append(shipyardOverviewResults(),
		fakeQueryResult{rows: fakeRowsFromValues([]any{2})},
		fakeQueryResult{rows: fakeRowsFromValues([]any{1})},
	)}
	...
	if statistics.Who != domaingame.StatisticsWhoAlly || len(statistics.Rows) != 0 {
		t.Fatalf("expected empty alliance statistics shell, got %+v", statistics)
	}
	if len(queryer.calls) != 6 {
		t.Fatalf("alliance mode should not run player row query yet, got calls=%+v", queryer.calls)
	}
}
...
func TestStatisticsRepositoryReturnsErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := repository.GetStatistics(context.Background(), appgame.StatisticsQuery{PlayerID: 42})
