...
function App() {
  ...
  useEffect(() => {
    ...
    for (const key of ["cp", "mode", "action", "fname", "player_id", "galaxy", "system", "filter", "modname", "from", "name", "id", "ip", "loca_src", "loca_dst"]) {
      const value = currentSearch.get(key);
      if (value) {
  ...
  const submitFleetLaunch = (draft: GameFleetDispatchLaunch) => {
    const publicSession = new URLSearchParams(search).get("session") ?? "";
    if (publicSession === "") {
      setGameFleetError("Session is invalid.");
      return;
    ...
    fetch(`/api/game/fleet?${fleetSearch.toString()}`, {
      ...
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
      body: JSON.stringify({ action: "launch-dispatch", ...draft })
    })
      .then(async (response) => {
        const text = await response.text();
        const payload = text ? (JSON.parse(text) as GameFleetStatus) : null;
