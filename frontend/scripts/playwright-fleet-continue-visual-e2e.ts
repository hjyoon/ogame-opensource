...
const outputDir = resolve(rootDir, ".tmp/playwright-fleet-continue-visual", browserName);
...
const fixturePath = resolve(process.env.OGAME_FLEET_CONTINUE_FIXTURE_FILE ?? join(rootDir, ".tmp/fleet-continue-fixture.json"));
...
const maxDiffRatio = numberEnv("OGAME_FLEET_CONTINUE_MAX_DIFF_RATIO", 0);
const colorDeltaThreshold = numberEnv("OGAME_FLEET_CONTINUE_COLOR_DELTA", 0);
const enforceDiff = process.env.OGAME_FLEET_CONTINUE_ENFORCE_DIFF !== "0";
...
try {
  ...
  const legacy = await captureFleetContinue(legacyContext, "legacy", legacyFleetURL(fixture), viewport);
  await legacyContext.close();

  const migratedContext = await newContext(browser, viewport, migratedBaseURL, fixture);
  const migrated = await captureFleetContinue(migratedContext, "migrated", migratedFleetURL(fixture), viewport);
  await migratedContext.close();

  const diffPath = join(screenshotDir, `fleet-continue-${viewport.name}-diff.png`);
  const diff = await compareScreenshots(browser, legacy.screenshotPath, migrated.screenshotPath, diffPath);
  const targetContractPass = JSON.stringify(visibleTargetContract(legacy.targetTable)) === JSON.stringify(visibleTargetContract(migrated.targetTable));
  const pass =
    legacy.status === 200 &&
    migrated.status === 200 &&
...
function legacyFleetURL(fixtureData: Fixture): string {
...
function migratedFleetURL(fixtureData: Fixture): string {
  const query = new URLSearchParams({
    session: fixtureData.session,
    cp: String(fixtureData.planet_id)
  });
...
async function captureFleetContinue(
  ...
  await page.locator(side === "legacy" ? "#content input[name='ship202']" : ".legacy-fleet-select-table input[name='ship202']").first().waitFor({
    timeout: 10_000
  });
  await waitForImages(page);
  await normalizeDynamicPageParts(page, side);
  await page.locator(side === "legacy" ? "#content input[name='ship202']" : ".legacy-fleet-select-table input[name='ship202']").first().fill("1");
  await page.locator(side === "legacy" ? "#content input[type='submit'][value='continue']" : ".legacy-fleet-select-table input[type='submit'][value='continue']").first().click();
  await page
    .locator(side === "legacy" ? "#content input[name='galaxy']" : ".legacy-fleet-target-table input[name='galaxy']")
    .first()
  ...
  const targetTable = await fleetTargetContract(page, side);
  const screenshotPath = join(screenshotDir, `fleet-continue-${viewportSpec.name}-${side}.png`);
...
async function fleetTargetContract(page: Page, side: "legacy" | "migrated"): Promise<PageCapture["targetTable"]> {
  return await page.evaluate((pageSide) => {
    const compact = (value: string | null | undefined): string => (value ?? "").replace(/\s+/g, " ").trim();
    const table =
      pageSide === "legacy"
        ? Array.from(document.querySelectorAll<HTMLTableElement>("#content table")).find((candidate) =>
            compact(candidate.textContent).includes("Departure of the fleet")
          )
        : document.querySelector<HTMLTableElement>(".legacy-fleet-target-table");
    if (!table) {
      return { text: "", box: null, rows: [], visibleRows: [] };
    }
    ...
    return {
      text: compact(table.textContent),
      box: { x: box.x, y: box.y, width: box.width, height: box.height },
      rows: rows.map((row) => compact(row.textContent)),
      visibleRows: rows.map((row) => visibleText(row))
    };
  }, side);
}

function visibleTargetContract(table: PageCapture["targetTable"]) {
  return {
...
function renderMarkdown(report: {
  ...
  const lines: string[] = [];
  lines.push(`# Fleet Continue Visual E2E (${report.browserName})`);
  lines.push("");
  ...
  lines.push(`Target contract pass: ${report.targetContractPass ? "yes" : "no"}`);
  ...
  lines.push("");
  lines.push("## Legacy Rows");
  for (const row of report.legacy.targetTable.rows) {
    lines.push(`- ${row}`);
  }
  lines.push("");
  lines.push("## Legacy Visible Rows");
  for (const row of report.legacy.targetTable.visibleRows) {
    lines.push(`- ${row}`);
  }
  lines.push("");
  lines.push("## Migrated Rows");
  for (const row of report.migrated.targetTable.rows) {
    lines.push(`- ${row}`);
  }
  lines.push("");
  lines.push("## Migrated Visible Rows");
  for (const row of report.migrated.targetTable.visibleRows) {
    lines.push(`- ${row}`);
  }
  return `${lines.join("\n")}\n`;
}
