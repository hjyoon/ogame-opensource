...
- Go serves `frontend/dist`, logs JSON through `log/slog`, and runs from `compose.golang.yaml` on port `8890`.
...
- `/api/game/{overview,buildings,resources,research,shipyard,defense,technology}` return game read models; resources also stores production settings, and technology supports `tid` details.
...
- Milestone: public parity, language flags, resource header caps, login redirect, research, shipyard, defense, and technology/detail read models are guarded.
...
- Playwright visual/CSR E2E covers public pages, language flags, game menu navigation, and auth overview/buildings/resources/research/shipyard/defense/technology/detail in Chromium and Firefox.
